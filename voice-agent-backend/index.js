    ...
    if (message && message.type === 'tool-calls') {
      ...
      for (const toolCall of toolCalls) {
        if (toolCall.function.name === 'create_calendar_event') {
          ...
          // DEBUG LOGGING - Check what Vapi sends
          ...
          const event = {
            summary: eventTitle,
            description: `Scheduled via Voice Agent | ${params.name}${params.title ? ` | ${params.title}` : ''}`,
            start: { 
              dateTime: startDateTime.toISOString(), 
              timeZone: 'America/Edmonton' // MST for Calgary
            },
            end: { 
          ...
          console.log('Creating event with title:', eventTitle);
          ...
          toolCallResponses.push({
            ...
            result: `Successfully scheduled "${eventTitle}". Event Link: ${calendarResponse.data.htmlLink}`,
          });
        }
      }
